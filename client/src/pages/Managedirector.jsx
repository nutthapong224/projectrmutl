...
const Managedirector = () => {
    ...
    const handleDelete = async (id) => {
        if (window.confirm('คุณแน่ใจหรือไม่ว่าต้องการลบข้อมูลนี้?')) {
            try {
                await axios.delete(`http://localhost:5000/api/director/player/${id}`);
    ...
    const exportToCSV = () => {
        ...
        try {
            ...
            link.setAttribute('download', `รายชื่อกรรมการ_${date}.csv`);
    ...
    const handleUpdateStatus = async () => {
        ...
        try {
            const response = await axios.put(`http://localhost:5000/api/director/update/${selectedUserId}`, {
    ...
    const handleClick = (id) => {
        navigate(`/admindashboard/updatedirector/${id}`);
    };
            ...
            <Grid container spacing={2} mb={3}>
                <Grid item>
                    <Button
                        variant="contained"
                        color="primary"
                        onClick={() => navigate("/admindashboard/adddirector")}
                    >
                        เพิ่มข้อมูล
                    </Button>
                </Grid>
                <Grid item>
                    <Button
                        variant="contained"
                        color="primary"
                        onClick={() => navigate("/admindashboard/registeradmindirector")}
                    >
                        เพิ่มข้อมูลแอดมินกรรมการ
                    </Button>
                </Grid>
                <Grid item>
                    <Button
                            ...
                            <TableBody>
                                {pendingUsers.map((user) => (
                                    <TableRow key={user.id}>
                                        ...
                                        >
                                            <img src={`http://localhost:5000${user.profile_image}`} alt="" width={50} />
                                        </TableCell>
                                        <TableCell>
