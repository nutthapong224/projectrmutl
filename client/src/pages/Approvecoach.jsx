...
const Approvecoach = () => {
  ...
  // เพิ่ม state สำหรับค้นหาชื่อและนามสกุล
  ...
  // API URLs
  const apiBaseUrl = import.meta.env.VITE_API_URL_APPROVECOACH;
  ...
  const exportToCSV = async () => {
    ...
    try {
      ...
      let url = `http://localhost:5000/api/coach/searchpending`;
      ...
      const fileName = `รายชื่อผู้คุมทีมรอการอนุมัติ_${formattedDate}.csv`;
  ...
  const exportToXLSX = async () => {
    ...
    try {
      ...
      XLSX.utils.book_append_sheet(workbook, worksheet, 'รายชื่อผู้คุมทีมรอการอนุมัติ');

      // สร้างชื่อไฟล์พร้อมวันที่
      const date = new Date();
      const formattedDate = `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}-${String(date.getDate()).padStart(2, '0')}`;
      const fileName = `รายชื่อผู้คุมทีมรอการอนุมัติ_${formattedDate}.xlsx`;
  ...
  const fetchPendingUsers = async () => {
    ...
    try {
      ...
      // เพิ่มการค้นหาด้วยชื่อและนามสกุล
      if (searchFirstName) {
        params.append('fname', searchFirstName);
  ...
  const handleResetFilters = () => {
    ...
    setCategories([]);
    setSearchFirstName(""); // รีเซ็ตค่าค้นหาชื่อ
    setSearchLastName(""); // รีเซ็ตค่าค้นหานามสกุล
  };
  ...
  useEffect(() => {
    ...
    fetchPendingUsers();
  }, [selectedDepartment, selectedSport, selectedPositionDirector, searchFirstName, searchLastName]); // เพิ่ม dependencies สำหรับการค้นหา
  ...
  return (
    <Container>
      <Typography variant="h4" gutterBottom>
        หน้าอนุมัติข้อมูลผู้คุมทีม
      </Typography>

      {/* Search Filters */}
      <Box sx={{ mb: 3, mt: 2 }}>
        <Grid container spacing={2} alignItems="center" mb={2}>
          {/* เพิ่มช่องค้นหาชื่อและนามสกุล */}
            ...
            <Table>
              <TableHead>
                <TableRow>
                  ...
                  <TableCell>รหัสลงทะเบียน</TableCell>
      ...
      <Modal open={imageModalOpen} onClose={closeImageModal}>
        <Box sx={{
          ...
          <img src={imageUrl} alt="User Image" style={{ maxWidth: '100%', maxHeight: '80vh' }} />
      ...
      <Modal open={pdfModalOpen} onClose={closePdfModal}>
        <Box sx={{
          ...
          <embed src={pdfUrl} width="100%" height="90%" />
          <Box sx={{ mt: 2, textAlign: 'right' }}>
