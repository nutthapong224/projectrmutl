...
const apiBaseUrl = import.meta.env.VITE_API_URL_APPROVEBADMINTON;
...
const UserTable = ({ users, onRefresh }) => {
  ...
  const handleUpdateStatus = async () => {
    try {
      const response = await axios.put(`${apiBaseUrl}/status/${selectedUserId}`, {
            ...
            {Array.isArray(users) && users.length > 0 ? (
              ...
              <TableRow>
                <TableCell colSpan={9} align="center">
                  {loading ? 'กำลังโหลดข้อมูล...' : 'ไม่มีข้อมูลผู้ใช้'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
...
const Updatamedalbadminton = () => {
  const [users, setUsers] = useState([]);

  const fetchUsers = async (searchResults = null) => {
    if (searchResults) {
...
export default Updatamedalbadminton;