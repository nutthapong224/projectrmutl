...
const Organizationscard = () => {
  ...
  useEffect(() => {
    const fetchPlayerDetails = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/api/studentorgranization/${id}`);
        ...
        <CardContent>
          ...
          {!flip && player && (
            <>
              ...
              >
                <span>ชื่อสกุล : </span>
                {player.prefix} {player.first_name} {player.last_name}
              </Typography>
              <Typography
                variant="body2"
                sx={{
              ...
              >
                <CardMedia
                  component="img"
                  height="160"
                  image={`http://localhost:5000/uploads/${player.photo_url}`}
