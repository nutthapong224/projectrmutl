...
exports.addPendingUser = async (req, res) => {
  ...
  const sql = `INSERT INTO pendingdirector 
  (fname, lname, phone_number, sport_type, position, created_at, department,  profile_image, document, title) 
  VALUES (?, ?, ?, ?, ?, NOW(), ?, ?, ?, ?)`;
...
exports.getPendingUsers = async (req, res) => {
  const sql = `
  SELECT r.registration_id, r.fname, r.lname, r.sport_table, c.category_name
  FROM pendingcoach r
  JOIN (
      SELECT 'football' AS sport, category_id, category_name FROM football
      UNION ALL
...
exports.searchPendingUsers = async (req, res) => {
  ...
  let sql = "SELECT * FROM pendingdirector WHERE 1=1";
  const params = [];
...
exports.searchUsers = async (req, res) => {
  ...
  // Start with base query that excludes soft-deleted records
  let sql = "SELECT * FROM director WHERE is_deleted = 0";
  const params = [];
  ...
  exports.searchUsersallvalue = async (req, res) => {
    ...
    let sql = "SELECT * FROM director WHERE 1=1";
    const params = [];
...
exports.approvePendingUser = async (req, res) => {
  ...
  try {
    ...
    const [pendingUser] = await pool.query(
      `SELECT 
       fname, lname, phone_number, sport_type, position, created_at, department,  profile_image, document, title
      FROM pendingdirector WHERE registration_id = ?`, // Changed 'id' to 'registration_id'
      [id]
    );
    ...
    const insertSql = `INSERT INTO director 
  (fname, lname, phone_number, sport_type, position, created_at, department,  profile_image, document, title) 
  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;
  ...
  await pool.query(insertSql, [
    ...
    userData.sport_type, // Corrected position
    userData.position,
    userData.created_at, // Ensure this is a valid datetime
    userData.department,
    ...
    await pool.query("DELETE FROM pendingdirector WHERE registration_id = ?", [id]);
...
exports.rejectPendingUser = async (req, res) => {
  ...
  try {
    const [result] = await pool.query("DELETE FROM pendingdirector WHERE id = ?", [id]);
...
exports.deleteUser = async (req, res) => {
  ...
  try {
    // Soft delete - update is_deleted flag instead of removing the record
    const [result] = await pool.query(
      "UPDATE director SET is_deleted = 1 WHERE registration_id = ?", 
      [id]
    );
...
exports.updatestatuss = async (req, res) => {
 ...
 exports.addUser= async (req, res) => {
    const {
      ...
      // คำสั่ง SQL สำหรับเพิ่มข้อมูลลงในตาราง studentorganization
      const sql = `INSERT INTO director 
      (fname, lname, phone_number, sport_type, position, created_at, department,  profile_image, document, title) 
      VALUES (?, ?, ?, ?, ?, NOW(), ?, ?, ?, ?)`;
...
exports.getplayerbyid = async (req, res) => {
  try {
    ...
    let sql = "SELECT * FROM director WHERE registration_id = ?"; // Fixed SQL query
...
exports.updateRegistration = async (req, res) => {
    try {
      const { id } = req.params;
      const { fname, lname, phone_number, sport_type, position, department, title } = req.body;
      ...
      let query = `
        UPDATE director
        SET fname = ?, lname = ?, phone_number = ?, sport_type = ?, position = ?, 
            department = ?, title = ?
      `;
  
      let values = [fname, lname, phone_number, sport_type, position, department, title];
