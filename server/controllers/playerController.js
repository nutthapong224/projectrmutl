...
exports.addPendingUser = async (req, res) => {
  ...
  (fname, lname, student_id, department, faculty, major, phone_number, sport_table, category_id, profile_image, document, create_at, status, title, typeteam) 
  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), ?, ?, ?)`;
...
exports.searchPendingUsers = async (req, res) => {
  ...
  let sql = `SELECT r.registration_id, r.department, r.fname, r.lname, r.document, 
    r.sport_table, c.category_name, r.profile_image, r.status, r.major, 
    r.phone_number, r.faculty, r.title, r.typeteam FROM registration r 
    JOIN (
      SELECT 'football' AS sport, category_id, category_name FROM football
      UNION ALL
    ...
    ) c ON r.sport_table = c.sport AND r.category_id = c.category_id 
    WHERE r.deleted = 0 `;
...
exports.searchUsersall = async (req, res) => {
  ...
  // สร้างตารางรวมจากกีฬาต่าง ๆ
  let sql = `SELECT r.registration_id, r.department, r.fname, r.lname, r.document,r.sport_table, c.category_name,r.profile_image, r.status, r.major,r.phone_number,r.faculty,r.title,r.typeteam
FROM registration r
JOIN (
    SELECT 'football' AS sport, category_id, category_name FROM football
    UNION ALL
...
exports.deleteUser = async (req, res) => {
  ...
  try {
    ...
    const [result] = await pool.query(
      "UPDATE registration SET deleted = 1, deleted_at = NOW() WHERE registration_id = ?", 
      [id]
    );
...
exports.updatestatuss = async (req, res) => {
 ...
 exports.addUser= async (req, res) => {
   ...
   const sql = `INSERT INTO registration
                (fname, lname, student_id, department, faculty, major, phone_number, sport_table, category_id, profile_image, document, created_at, status,title,typeteam) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), ?,?,?)`;
 ...
 exports.getplayerbyid = async (req, res) => {
  try {
    ...
    let sql = `
      SELECT r.registration_id, r.department, r.fname, r.lname, r.document, r.sport_table, 
             c.category_name, r.profile_image, r.status, r.major, r.phone_number, 
             r.faculty, r.title,r.student_id,r.typeteam
      FROM registration r
      JOIN (
          SELECT 'football' AS sport, category_id, category_name FROM football
          UNION ALL
...
exports.updateRegistration = async (req, res) => {
  try {
    const { id } = req.params;
    const { fname, lname, student_id, phone_number, sport_table, category_id, department, faculty, major, status, title,typeteam } = req.body;
    ...
    let query = `
      ...
      SET fname = ?, lname = ?, student_id = ?, phone_number = ?, sport_table = ?, category_id = ?, 
          department = ?, faculty = ?, major = ?, status = ?, title = ?, typeteam=?
    `;
    let values = [fname, lname, student_id, phone_number, sport_table, category_id, department, faculty, major, status, title,typeteam];
