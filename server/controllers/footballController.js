...
const addPendingUser = async (req, res) => {
  ...
  const sql = `INSERT INTO pendingfootball (prefix, fname, lname, student_id, department, faculty, major, phone_number, sport_type, profile_image, document, created_at,status)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(),?)`;
...
const getPendingFilters = async (req, res) => {
  try {
    const [departments] = await pool.query("SELECT DISTINCT department FROM pendingfootball WHERE department IS NOT NULL");
    const [sportTypes] = await pool.query("SELECT DISTINCT sport_type FROM pendingfootball WHERE sport_type IS NOT NULL");
...
const getFilters = async (req, res) => {
  try {
    const [departments] = await pool.query("SELECT DISTINCT department FROM football WHERE department IS NOT NULL");
    const [sportTypes] = await pool.query("SELECT DISTINCT sport_type FROM football WHERE sport_type IS NOT NULL");
...
const updatePendingStatus = async (req, res) => {
  ...
  try {
    ...
    const sql = "UPDATE football SET status = ?, updated_at = NOW() WHERE id = ?";
...
const getPendingStatus = async (req, res) => {
  try {
    const sql = "SELECT id, fname, lname, status FROM pendingfootball";
...
const searchPendingUsers = async (req, res) => {
  ...
  let sql = "SELECT * FROM pendingfootball WHERE 1=1";
  const values = [];
...
const getpenddingAllUsers = async (req, res) => {
  const sql = "SELECT * FROM pendingfootball";
...
const approvePendingUser = async (req, res) => {
  ...
  try {
    ...
    const [pendingUser] = await pool.query(
      "SELECT * FROM pendingfootball WHERE id = ?",
      [id]
    );
    ...
    const insertSql = `INSERT INTO football (prefix, fname, lname, student_id, department, faculty, major, phone_number, sport_type, profile_image, document, created_at, status)
                       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?)`;
    ...
    await pool.query("DELETE FROM pendingfootball WHERE id = ?", [id]);

    res.status(200).send({ message: "User approved and moved to football." });
  } catch (err) {
    console.error(err);
...
const rejectPendingUser = async (req, res) => {
  ...
  try {
    const [result] = await pool.query("DELETE FROM pendingfootball WHERE id = ?", [id]);
...
const addUser = async (req, res) => {
  ...
  const sql = `INSERT INTO football (prefix, fname, lname, student_id, department, faculty, major, phone_number, sport_type, profile_image, document, created_at, updated_at, status)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW(),?)`;
...
const getAllUsers = async (req, res) => {
  const sql = "SELECT * FROM football";
