...
const addPendingUser = async (req, res) => {
  ...
  const sql = `INSERT INTO pendingbasketball (prefix, fname, lname, student_id, department, faculty, major, phone_number, sport_type, profile_image, document, created_at,status)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(),?)`;
...
const getPendingFilters = async (req, res) => {
  try {
    const [departments] = await pool.query("SELECT DISTINCT department FROM pendingbasketball WHERE department IS NOT NULL");
    const [sportTypes] = await pool.query("SELECT DISTINCT sport_type FROM pendingbasketball WHERE sport_type IS NOT NULL");
...
const getFilters = async (req, res) => {
  try {
    const [departments] = await pool.query("SELECT DISTINCT department FROM basketball WHERE department IS NOT NULL");
    const [sportTypes] = await pool.query("SELECT DISTINCT sport_type FROM basketball WHERE sport_type IS NOT NULL");
...
const updatePendingStatus = async (req, res) => {
  ...
  try {
    ...
    const sql = "UPDATE basketball SET status = ?, updated_at = NOW() WHERE id = ?";
...
const getPendingStatus = async (req, res) => {
  try {
    const sql = "SELECT id, fname, lname, status FROM pendingbasketball";
...
const searchPendingUsers = async (req, res) => {
  ...
  let sql = "SELECT * FROM pendingebasketball WHERE 1=1";
  const values = [];
...
const getpenddingAllUsers = async (req, res) => {
  const sql = "SELECT * FROM pendingbasketball";
...
const approvePendingUser = async (req, res) => {
  ...
  try {
    ...
    const [pendingUser] = await pool.query(
      "SELECT * FROM pendingbasketball WHERE id = ?",
      [id]
    );
    ...
    const insertSql = `INSERT INTO basketball (prefix, fname, lname, student_id, department, faculty, major, phone_number, sport_type, profile_image, document, created_at, status)
                       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?)`;
    ...
    await pool.query("DELETE FROM pendingbasketball WHERE id = ?", [id]);

    res.status(200).send({ message: "User approved and moved to basketball." });
  } catch (err) {
    console.error(err);
...
const rejectPendingUser = async (req, res) => {
  ...
  try {
    const [result] = await pool.query("DELETE FROM pendingbasketball WHERE id = ?", [id]);
...
const addUser = async (req, res) => {
  ...
  const sql = `INSERT INTO basketball (prefix, fname, lname, student_id, department, faculty, major, phone_number, sport_type, profile_image, document, created_at, updated_at, status)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW(),?)`;
...
const getAllUsers = async (req, res) => {
  const sql = "SELECT * FROM basketball";
