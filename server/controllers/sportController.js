...
exports.searchPendingUsers = async (req, res) => {
  ...
  let sql = `SELECT r.registration_id, r.department, r.document,r.title,r.phone_number,r.fname, r.student_id,r.lname, r.sport_table, c.category_name,r.profile_image, r.status,r.typeteam
FROM pendingregistration r
JOIN (
    SELECT 'football' AS sport, category_id, category_name FROM football
    UNION ALL
  ...
  try {
    ...
    const [pendingUser] = await pool.query(
      `SELECT 
        ...
        document, status, title,typeteam
    ...
    const insertSql = `INSERT INTO registration (fname, lname, student_id, phone_number, sport_table, category_id, created_at, department, faculty, major, profile_image, document, status, title,typeteam)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?)`;
...
exports.rejectPendingUser = async (req, res) => {
  ...
  try {
    const [result] = await pool.query("DELETE FROM pendingregistration WHERE registration_id = ?", [id]);
...
exports.updatestatus = async () => {
  ...
  const query = "UPDATE your_table SET status = ? WHERE registration_id = ?";
  db.query(query, [status, registration_id], (err, result) => {
    if (err) {
