...
const addPendingUser = async (req, res) => {
  ...
  const sql = `INSERT INTO pendingbadminton (prefix, fname, lname, student_id, department, faculty, major, phone_number, sport_type, profile_image, document, created_at,status)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(),?)`;
...
const getPendingFilters = async (req, res) => {
  try {
    const [departments] = await pool.query("SELECT DISTINCT department FROM pendingbadminton WHERE department IS NOT NULL");
    const [sportTypes] = await pool.query("SELECT DISTINCT sport_type FROM pendingbadminton WHERE sport_type IS NOT NULL");
...
const getFilters = async (req, res) => {
  try {
    const [departments] = await pool.query("SELECT DISTINCT department FROM badminton WHERE department IS NOT NULL");
    const [sportTypes] = await pool.query("SELECT DISTINCT sport_type FROM badminton WHERE sport_type IS NOT NULL");
...
const updatePendingStatus = async (req, res) => {
  ...
  try {
    // SQL query to update the 'pendingbadminton' table with the new status
    const sql = "UPDATE badminton SET status = ?, updated_at = NOW() WHERE id = ?";
...
// API ดึงข้อมูล status ของ pendingbadminton
const getPendingStatus = async (req, res) => {
  try {
    const sql = "SELECT id, fname, lname, status FROM pendingbadminton";
...
const searchPendingUsers = async (req, res) => {
  ...
  let sql = "SELECT * FROM pendingbadminton WHERE 1=1";
  const values = [];
...
const getpenddingAllUsers = async (req, res) => {
  const sql = "SELECT * FROM pendingbadminton";
...
const approvePendingUser = async (req, res) => {
  ...
  try {
    // Fetch user data from pendingbadminton
    const [pendingUser] = await pool.query(
      "SELECT * FROM pendingbadminton WHERE id = ?",
      [id]
    );
    ...
    // Insert user data into badminton table
    const insertSql = `INSERT INTO badminton (prefix, fname, lname, student_id, department, faculty, major, phone_number, sport_type, profile_image, document, created_at, status)
                       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?)`;
    ...
    // Delete user data from pendingbadminton
    await pool.query("DELETE FROM pendingbadminton WHERE id = ?", [id]);

    res.status(200).send({ message: "User approved and moved to badminton." });
  } catch (err) {
    console.error(err);
...
const rejectPendingUser = async (req, res) => {
  ...
  try {
    const [result] = await pool.query("DELETE FROM pendingbadminton WHERE id = ?", [id]);
...
const addUser = async (req, res) => {
  ...
  const sql = `INSERT INTO badminton (prefix, fname, lname, student_id, department, faculty, major, phone_number, sport_type, profile_image, document, created_at, updated_at, status)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW(),?)`;
