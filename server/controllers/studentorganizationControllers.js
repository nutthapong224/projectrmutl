...
exports.addPendingUser = async (req, res) => {
  ...
  const sql = `INSERT INTO pendingstudentorganization 
               (fname, lname, student_id, phone_number, department, faculty, major, profile_image, document, created_at, status, title, position) 
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), ?, ?, ?)`;
...
exports.searchPendingUsers = async (req, res) => {
  const { position, department,fname,lname} = req.query;
  
  let sql = "SELECT * FROM pendingstudentorganization WHERE 1=1";
  const params = [];
...
exports.searchUsers = async (req, res) => {
  const { position, department, fname, lname, includeDeleted } = req.query;
  
  // Base query - exclude deleted users by default
  let sql = "SELECT * FROM studentorganization WHERE 1=1";
  
  // Only show active users (not deleted) unless includeDeleted flag is set to true
...
exports.approvePendingUser = async (req, res) => {
  ...
  try {
    ...
    const [pendingUser] = await pool.query(
      `SELECT 
       fname, lname, student_id, phone_number, department, faculty, major, profile_image, document,created_at,status, title, position
      FROM pendingstudentorganization WHERE registration_id = ?`, // Changed 'id' to 'registration_id'
      [id]
    );
    ...
    const insertSql = `INSERT INTO studentorganization 
    (fname, lname, student_id, phone_number, department, faculty, major, profile_image, document, created_at, status, title, position) 
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;
...
userData.position
    ...
    await pool.query("DELETE FROM pendingstudentorganization WHERE registration_id = ?", [id]);
...
exports.rejectPendingUser = async (req, res) => {
  ...
  try {
    const [result] = await pool.query("DELETE FROM pendingstudentorganization WHERE id = ?", [id]);
...
exports.deleteUser = async (req, res) => {
  ...
  try {
    // Instead of deleting, update the is_deleted flag and record deletion time
    const [result] = await pool.query(
      "UPDATE studentorganization SET is_deleted = 1, deleted_at = NOW() WHERE registration_id = ? AND is_deleted = 0",
      [id]
    );
...
exports.updatestatuss = async (req, res) => {
 ...
 exports.addUser= async (req, res) => {
  ...
  // คำสั่ง SQL สำหรับเพิ่มข้อมูลลงในตาราง studentorganization
  const sql = `INSERT INTO studentorganization 
               (fname, lname, student_id, phone_number, department, faculty, major, profile_image, document, created_at, status, title, position) 
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), ?, ?, ?)`;
...
exports.getplayerbyid = async (req, res) => {
  try {
    ...
    let sql = "SELECT * FROM studentorganization WHERE registration_id = ?"; // Fixed SQL query
...
exports.updateRegistration = async (req, res) => {
  try {
    ...
    let query = `
      ...
      SET fname = ?, lname = ?, student_id = ?, phone_number = ?,  department = ?, faculty = ?, major = ?, status = ?, title = ?, position = ?
    `;
    let values = [    
      fname,
      lname,
