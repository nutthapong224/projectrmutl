...
const addPendingUser = async (req, res) => {
  ...
  const sql = `INSERT INTO pendingtabletenis (prefix, fname, lname, student_id, department, faculty, major, phone_number, sport_type, profile_image, document, created_at,status)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(),?)`;
...
const getPendingFilters = async (req, res) => {
  try {
    const [departments] = await pool.query("SELECT DISTINCT department FROM pendingtabletenis WHERE department IS NOT NULL");
    const [sportTypes] = await pool.query("SELECT DISTINCT sport_type FROM pendingtabletenis WHERE sport_type IS NOT NULL");
...
const getFilters = async (req, res) => {
  try {
    const [departments] = await pool.query("SELECT DISTINCT department FROM tabletenis WHERE department IS NOT NULL");
    const [sportTypes] = await pool.query("SELECT DISTINCT sport_type FROM tabletenis WHERE sport_type IS NOT NULL");
...
const updatePendingStatus = async (req, res) => {
  ...
  try {
    ...
    const sql = "UPDATE tabletenis SET status = ?, updated_at = NOW() WHERE id = ?";
...
const getPendingStatus = async (req, res) => {
  try {
    const sql = "SELECT id, fname, lname, status FROM pendingtabletenis";
...
const searchPendingUsers = async (req, res) => {
  ...
  let sql = "SELECT * FROM pendingetabletenis WHERE 1=1";
  const values = [];
...
const searchUserss = async (req, res) => {
  ...
  let sql = "SELECT * FROM tabletenis WHERE 1=1";
  const values = [];
...
const getpenddingAllUsers = async (req, res) => {
  const sql = "SELECT * FROM pendingtabletenis";
...
const approvePendingUser = async (req, res) => {
  ...
  try {
    ...
    const [pendingUser] = await pool.query(
      "SELECT * FROM pendingtabletenis WHERE id = ?",
      [id]
    );
    ...
    const insertSql = `INSERT INTO tabletenis (prefix, fname, lname, student_id, department, faculty, major, phone_number, sport_type, profile_image, document, created_at, status)
                       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?)`;
    ...
    await pool.query("DELETE FROM pendingtabletenis WHERE id = ?", [id]);

    res.status(200).send({ message: "User approved and moved to tabletenis." });
  } catch (err) {
    console.error(err);
...
const rejectPendingUser = async (req, res) => {
  ...
  try {
    const [result] = await pool.query("DELETE FROM pendingtabletenis WHERE id = ?", [id]);
...
const addUser = async (req, res) => {
  ...
  const sql = `INSERT INTO tabletenis (prefix, fname, lname, student_id, department, faculty, major, phone_number, sport_type, profile_image, document, created_at, updated_at, status)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW(),?)`;
...
const getUserById = async (req, res) => {
  const { id } = req.params;
  const sql = "SELECT * FROM tabletenis WHERE id = ?";
...
const getAllUsers = async (req, res) => {
  const sql = "SELECT * FROM tabletenis";
