...
const addPendingUser = async (req, res) => {
  ...
  const sql = `INSERT INTO pendingtakraw (prefix, fname, lname, student_id, department, faculty, major, phone_number, sport_type, profile_image, document, created_at,status)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(),?)`;
...
const getPendingFilters = async (req, res) => {
  try {
    const [departments] = await pool.query("SELECT DISTINCT department FROM pendingtakraw WHERE department IS NOT NULL");
    const [sportTypes] = await pool.query("SELECT DISTINCT sport_type FROM pendingtakraw WHERE sport_type IS NOT NULL");
...
const getFilters = async (req, res) => {
  try {
    const [departments] = await pool.query("SELECT DISTINCT department FROM takraw WHERE department IS NOT NULL");
    const [sportTypes] = await pool.query("SELECT DISTINCT sport_type FROM takraw WHERE sport_type IS NOT NULL");
...
const updatePendingStatus = async (req, res) => {
  ...
  try {
    ...
    const sql = "UPDATE takraw SET status = ?, updated_at = NOW() WHERE id = ?";
...
const getPendingStatus = async (req, res) => {
  try {
    const sql = "SELECT id, fname, lname, status FROM pendingtakraw";
...
const searchPendingUsers = async (req, res) => {
  ...
  let sql = "SELECT * FROM pendingetakraw WHERE 1=1";
  const values = [];
...
const searchUserss = async (req, res) => {
  ...
  let sql = "SELECT * FROM takraw WHERE 1=1";
  const values = [];
...
const getpenddingAllUsers = async (req, res) => {
  const sql = "SELECT * FROM pendingtakraw";
...
const approvePendingUser = async (req, res) => {
  ...
  try {
    ...
    const [pendingUser] = await pool.query(
      "SELECT * FROM pendingtakraw WHERE id = ?",
      [id]
    );
    ...
    const insertSql = `INSERT INTO takraw (prefix, fname, lname, student_id, department, faculty, major, phone_number, sport_type, profile_image, document, created_at, status)
                       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?)`;
    ...
    await pool.query("DELETE FROM pendingtakraw WHERE id = ?", [id]);

    res.status(200).send({ message: "User approved and moved to takraw." });
  } catch (err) {
    console.error(err);
...
const rejectPendingUser = async (req, res) => {
  ...
  try {
    const [result] = await pool.query("DELETE FROM pendingtakraw WHERE id = ?", [id]);
...
const addUser = async (req, res) => {
  ...
  const sql = `INSERT INTO takraw (prefix, fname, lname, student_id, department, faculty, major, phone_number, sport_type, profile_image, document, created_at, updated_at, status)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW(),?)`;
...
const getUserById = async (req, res) => {
  const { id } = req.params;
  const sql = "SELECT * FROM takraw WHERE id = ?";
