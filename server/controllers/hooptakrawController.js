...
const addPendingUser = async (req, res) => {
  ...
  const sql = `INSERT INTO pendinghooptakraw (prefix, fname, lname, student_id, department, faculty, major, phone_number, sport_type, profile_image, document, created_at,status)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(),?)`;
...
const getPendingFilters = async (req, res) => {
  try {
    const [departments] = await pool.query("SELECT DISTINCT department FROM pendinghooptakraw WHERE department IS NOT NULL");
    const [sportTypes] = await pool.query("SELECT DISTINCT sport_type FROM pendinghooptakraw WHERE sport_type IS NOT NULL");
...
const getFilters = async (req, res) => {
  try {
    const [departments] = await pool.query("SELECT DISTINCT department FROM hooptakraw WHERE department IS NOT NULL");
    const [sportTypes] = await pool.query("SELECT DISTINCT sport_type FROM hooptakraw WHERE sport_type IS NOT NULL");
...
const updatePendingStatus = async (req, res) => {
  ...
  try {
    ...
    const sql = "UPDATE hooptakraw SET status = ?, updated_at = NOW() WHERE id = ?";
...
const getPendingStatus = async (req, res) => {
  try {
    const sql = "SELECT id, fname, lname, status FROM pendinghooptakraw";
...
const searchPendingUsers = async (req, res) => {
  ...
  let sql = "SELECT * FROM pendingehooptakraw WHERE 1=1";
  const values = [];
...
const searchUserss = async (req, res) => {
  ...
  let sql = "SELECT * FROM hooptakraw WHERE 1=1";
  const values = [];
...
const getpenddingAllUsers = async (req, res) => {
  const sql = "SELECT * FROM pendinghooptakraw";
...
const approvePendingUser = async (req, res) => {
  ...
  try {
    ...
    const [pendingUser] = await pool.query(
      "SELECT * FROM pendinghooptakraw WHERE id = ?",
      [id]
    );
    ...
    const insertSql = `INSERT INTO hooptakraw (prefix, fname, lname, student_id, department, faculty, major, phone_number, sport_type, profile_image, document, created_at, status)
                       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?)`;
    ...
    await pool.query("DELETE FROM pendinghooptakraw WHERE id = ?", [id]);

    res.status(200).send({ message: "User approved and moved to hooptakraw." });
  } catch (err) {
    console.error(err);
...
const rejectPendingUser = async (req, res) => {
  ...
  try {
    const [result] = await pool.query("DELETE FROM pendinghooptakraw WHERE id = ?", [id]);
...
const addUser = async (req, res) => {
  ...
  const sql = `INSERT INTO hooptakraw (prefix, fname, lname, student_id, department, faculty, major, phone_number, sport_type, profile_image, document, created_at, updated_at, status)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW(),?)`;
...
const getUserById = async (req, res) => {
  const { id } = req.params;
  const sql = "SELECT * FROM hooptakraw WHERE id = ?";
...
const getAllUsers = async (req, res) => {
  const sql = "SELECT * FROM hooptakraw";
